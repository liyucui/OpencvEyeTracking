...
def multiplyProbImages(newProb, priorToMultiply, YXoffsetOfSecondWithinFirst, defaultPriorValue):
    if np.any(YXoffsetOfSecondWithinFirst > newProb.shape) or np.any(-YXoffsetOfSecondWithinFirst > priorToMultiply.shape):
        ...
        print("YXoffsetOfSecondWithinFirst: ", YXoffsetOfSecondWithinFirst)
        print("newProb.shape: ", newProb.shape)
        print("priorToMultiply.shape: ", priorToMultiply.shape)
...
##        eye = leftEye_rightEye[1]
            ...
            if allowDebugDisplay:
                ...
                # cv2.circle(output, (int(pupilXYList[eyeIndex][0]), int(pupilXYList[eyeIndex][0])), 3, (255,0,0),thickness=1) #BGR format
...
##        cv2.circle(output, (cornerCx+corner[0],cornerCy+corner[1]), 2, (255,255,0),thickness=1) #BGR format
        ...
        if directInferenceLeftRight:
            ...
            if allowDebugDisplay:
                ...
                cv2.line(output, (int(pupilXYList[0][0]),int(pupilXYList[0][1])), (int(pupilXYList[1][0]), int(pupilXYList[1][1])), (0,100,100))
...
##            import pdb; pdb.set_trace()
            if allowDebugDisplay:
                # 虚拟点与视线的连线？
                cv2.line(output, (int(refXY[0]),int(refXY[1])), (int(refXY[0]+pupilXYList[0]), int(refXY[1]+pupilXYList[1])), (0,255,255))
